...
interface PlayerCardProps {
  ...
  compact?: boolean;
  className?: string;
  /** Optional mapper to override the displayed role label (e.g., map to Slot 1-4) */
...
const PlayerCard: React.FC<PlayerCardProps> = ({
  ...
  const getRoleAvatarGradient = (role: string) => {
    ...
    // Use gold primary palette for Batsman
    if (r === "batsman" || r === "batsmen") return "bg-gradient-to-br from-primary-400 to-primary-600";
  ...
  const displayRole = displayRoleMap ? displayRoleMap(player.role) : player.role;
          ...
          <div className="flex space-x-2 mt-4 pt-3 border-t border-gray-100">
            {onSetCaptain && !isCaptain && (
              <button
                ...
                className="flex-1 px-3 py-1.5 text-xs font-medium text-primary-700 bg-primary-50 border border-primary-200 rounded-lg hover:bg-primary-100"
              >
                Make Captain
              </button>
            )}
